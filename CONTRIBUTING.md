...
### Tracking SHA updates
In general SHA updates in RPCO are done every two weeks. When the update is done, the issues in Pending-SHA-update that are resolved by the update, should be moved to Done once the task tracking the SHA update is merged.
